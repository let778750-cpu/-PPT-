...
深度学习课程实验自动化 agent 的项目总控文件。
8 阶段流水线、11 个 skill、`_workflow/` 状态追踪系统。
...
├── 实验要求/                       # 每个实验的 PDF 与可选伴随数据资产
│   └── 实验N要求/                   # 实验 N 的 PDF、压缩包、npz、csv 等本地资产
├── 实验报告模板_latex/              # main.tex + 报告样式文件
├── PPT生成skill/                   # ppt-master 或其他 PPT 生成工具（需用户自行配置）
...
- Conda 环境：用户自定义的深度学习环境（建议 Python 3.10+, PyTorch, CUDA）
- Python 完整路径：用户自行配置（如 `/path/to/conda/envs/your_env/python.exe`）
- 命令前缀：`/path/to/python -s -X utf8`
  - `-s`：防止 user-site 覆盖 Conda 包路径
  - `-X utf8`：确保中文 stdout 正常输出
  - **不可省略这两个标志**
...
执行实验流水线时必须推进到 Phase 8 PPT 生成与复核后才算完整交付。

## 权威源优先级

多源冲突时按此顺序裁决：

1. 实验要求 PDF 及同目录伴随数据资产（`实验要求/实验N要求/`）
2. 已验证的代码和测试结果
3. README 等说明文件
...
- 环境检查：运行 Python 验证 `torch.cuda.is_available()`
...
- **实验要求目录可能含数据资产**：必须先枚举 `实验N要求/` 同目录文件；已有数据集优先使用
- **不要盲目信任 PDF 中的示例代码**：可能含有意或无意的错误，必须验证
- **Conda 环境不是系统默认 Python**：必须使用完整路径，不能用 `python` 直接调用
- **`-s` 标志不可省略**：省略会导致 user-site 下的包覆盖 Conda 环境，引发 import 错误
