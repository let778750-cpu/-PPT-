...
模板文件 `实验报告模板_latex/main.tex` 的最终报告应包含五个一级部分：

1. **一、实验内容** — 实验目标、输入、任务描述和验收要求；不得放置公式推导
2. **二、实验原理** — 与本实验相关的理论背景、算法原理、数学公式
3. **三、实验算法设计** — 算法设计思路、关键实现细节、代码说明
4. **四、实验结果与分析** — 实验结果、指标分析、目标对比、误差解释和可视化分析
5. **五、总结与讨论** — 包含 `5.1 实验总结` 与 `5.2 讨论`；总结回顾实验全貌，讨论按顶会/顶刊实验讨论段的风格辩证说明局限性、可优化方向、应用场景和未来深入研究路径
...
- 实验原理中定义的符号，在算法设计、实验结果与分析、总结与讨论中必须保持一致
...
- 正文插图必须放在 `figure` 环境中并使用 `\centering`；图像、正文和图注之间不得出现由大画布或浮动间距造成的异常空白。
- 实验原理部分如果解释网络结构、注意力机制、卷积/池化、RNN/LSTM 门控、分割流程等视觉概念，必须配一张原理图或结构图，而不是只写文字。
- 导出的流程图/结构图必须裁掉多余白边，报告中优先引用紧凑边界的 PDF；若使用 PNG，图像本身白边不得明显大于内容区。

### 标题与编号

- 每张图必须有中文标题，最终 PDF 中应呈现为："图 2.1. 描述内容"、"图 3.1. 描述内容"等“章节号.图号”格式；图号随 `\experimentsectionpage` 重置。
- 每个表必须有中文标题，最终 PDF 中应按全文顺序呈现为："表 1. 描述内容"、"表 2. 描述内容"等格式，不按章节重置。
...
```latex
\DeclareCaptionLabelFormat{compactlabel}{#1#2}
\captionsetup{labelformat=compactlabel,labelsep=period,font=small,labelfont=bf,skip=4pt}
\renewcommand{\figurename}{图}
\renewcommand{\tablename}{表}
\newcounter{experimentsection}
\renewcommand{\thefigure}{\arabic{experimentsection}.\arabic{figure}}
...
并在 `\experimentsectionpage` 中执行 `\stepcounter{experimentsection}` 与 `\setcounter{figure}{0}`。
...
- 图中文字使用中文
- 节点形状区分语义：矩形=处理步骤，菱形=判断，圆角矩形=开始/结束或模块
- 流程方向自上而下或自左而右，保持一致；连接线必须横平竖直，使用直角折线，不得使用弯曲线
- 箭头清晰，避免交叉；连接线不得覆盖节点、文字、公式或图注
- 框内文字必须自动换行并留出内边距，严禁出现文字越出边框、贴边或压线
- 推荐采用白色背景、浅灰线条、低饱和 pastel 色块、细描边和紧凑留白，接近学术论文中的系统框图风格
...
- 结果分析：哪些设计选择有效，为什么
- 误差解释：未达标项、边界样例、异常曲线或失败尝试必须如实说明

### 五、总结与讨论（约 1 页）

本部分必须包含两个二级小模块，且篇幅基本相当：

- **5.1 实验总结**：概括整个实验全貌，覆盖实验目标回顾、核心方法回顾、关键结果回顾、整体收获与意义。不能只重复第四部分的指标表。
- **5.2 讨论**：结合真实结果，按顶会/顶刊实验讨论风格说明局限性与优化空间、应用方向、未来深入研究路径。不得空泛拔高 novelty。
- 篇幅平衡：较短模块原则上不低于较长模块的 80%；若某实验确有特殊原因，应在正文中自然说明侧重点。
...
- **标题间距一致**：参考文献标题与下方文献条目之间的间距必须与前五个章节标题与正文之间的间距严格一致（均为 1em，由 `\experimentsectionpage` 宏控制）
- **单标题**：不得同时出现 `\experimentsectionpage{参考文献}` 标题和 `thebibliography` 默认居中标题；使用 `\renewenvironment{thebibliography}` 移除默认标题。
