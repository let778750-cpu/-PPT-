...
- 渲染后的 PDF 图表是否居中且没有超过正文左右页边距；流程图/模型结构图不得出现箭头贴框、压框、穿框或与框线重合。
...
- 章节结构是否统一为“第四部分：实验结果与分析”“第五部分：总结与讨论”，且第五部分包含 `5.1 实验总结` 与 `5.2 讨论`。
...
- 参考文献标题与首条文献之间的间距是否与前五个章节标题与正文的间距一致（检查是否通过 `\renewenvironment{thebibliography}` 移除了内置 `\section*`，而非仅用 `\renewcommand{\refname}{}`）。
...
- `5.1 实验总结` 是否覆盖实验目标回顾、核心方法回顾、关键结果回顾和整体收获；`5.2 讨论` 是否覆盖局限性、优化空间、应用方向和未来研究。
- `5.1 实验总结` 与 `5.2 讨论` 篇幅是否基本相当，较短者原则上不低于较长者的 80%。
...
- 页码来源不完整但不影响核心结论。
- 图表质量不足或缺少流程图。
- 流程图/模型结构图因横向空间不足导致箭头与元素框重合，或为放大图像而越过正文页边距。
- 图表仍使用可替代的低清晰度位图。
- 图表编号分隔符不是句点。
- 第一部分出现应放入实验原理的公式推导。
- 代码片段过长或说明不足。
- 结论表述太口语化、缺少定量分析。
- 参考文献未另起一页，或"参考文献"标题未左对齐，或标题与文献条目间距与前五章节不一致。
...
- 参考文献中存在非权威或疑似虚构的引用。
- 第四、第五部分仍沿用旧结构（如“四、实验总结”“五、讨论与展望”），或缺少 `5.1 实验总结` / `5.2 讨论`。
- `5.1 实验总结` 缺少全实验回顾，或 `5.2 讨论` 缺少真实局限、应用方向和未来研究，或两者篇幅明显失衡。
